...
##dataset feature preparation for different models:
...
##for leave one sample out cross validation: + training() / validate2()
LOSOCV = Training.getLOSOCV('DS', 'SMIC', []) #using 'DS'/'DS_domain'; 'SAMM'/'SMIC'/; []/'CK+'; # 'DS','SAMM'/'SMIC',[]; 'DS_domain','SAMM'/'SMIC','CK+'

##for training:
# modelSMIC=Training.training(LOSOCV, weightspathdir)
print("done training")

##for model parameter info:
# print(modelSMIC.summary())
...
# print_flops(modelSMIC)

##for label path test:
# SMIC = Prepare.prepareSMICtest(MM, OF, fps, SMIC_dataset_path)
