...
def prepareSMICtest(MM, OF, fps, SMIC_dataset_path):
    SMIC = []
    SMIC_list = []
    SMIC_labels = []
    SMIC_flow_list = []
        ...
        for label in range(3):
            ...
            for example in os.listdir(path[label]):
                ...
                for frame in range(framerange):
                    ...
                    # save image path: SMIC/HS/s1/micro/negative/s1_ne_01
                    print('frame image path:')
                    print(imagepath)   
                    # df = pandas.DataFrame(imagepath)
                    # df.to_csv('SMIC_imagepath_frame.txt')
                ...
                for duplicate in range(18 - framerange):
                    ...
                    # save image path: SMIC/HS/s1/micro/negative/s1_ne_01
                    print('duplicate image path:')
                    print(imagepath)    
                    # df = pandas.DataFrame(imagepath)
                    # df.to_csv('SMIC_imagepath_duplicate.txt')
